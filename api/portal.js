// Vercel Serverless Function — Portal 免登入身份驗證
//
// 安全原則（與 /api/proxy 一致）：
...
//   5. 角色必須同時在「旅團 portalRoles 白名單」與「系統已知角色」內
//      （member 亦可經 Portal 進入：只讀自己進度，寫入照樣由旅團 GAS 驗權）
...
// 與姊妹系統 vsbadge /api/portal 同結構（svc 名與測試開關改為 roverbadge）。

import { getTrustedTroop, normalizeOrigin } from './_registry.js';

// 系統已知角色（member 是 Portal 主要使用者：只讀自己進度，勾選權限由前端
// can_tick＋旅團 GAS 把關，所以唔可以用「可勾選角色」嚟擋 member 入口）
const KNOWN_ROLES = ['member', 'admin', 'group_leader', 'branch_leader', 'exec_committee', 'super_admin'];
// 旅團沒設 portalRoles 時的預設白名單（最小權限）
const DEFAULT_PORTAL_ROLES = ['exec_committee'];

// 本機／預覽環境測試專用：設 ROVERBADGE_PORTAL_TEST=1 時放寬來源檢查（讓任何 origin 都能試 portal 流程）。
// 雙重保護：只要跑在 Vercel（VERCEL=1，正式與 Preview 部署皆然）就必定失效，
// 因此即使誤把這個 env 加到 Vercel 專案，也不會在生產環境開洞。
const PORTAL_TEST = process.env.ROVERBADGE_PORTAL_TEST === '1' && process.env.VERCEL !== '1';
...
function safeLog(fields) {
  ...
  try { console.log(JSON.stringify({ svc: 'roverbadge-portal', ...fields })); } catch (e) { /* ignore */ }
}
...
export default function handler(req, res) {
  ...
  if (!allowed.includes(role) || !KNOWN_ROLES.includes(role)) {
