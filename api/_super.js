...
export { accountId };
export const isSuperId = id => typeof id === 'string' && [accountId, `${accountId}@roverbadge.local`].includes(id.trim().toLowerCase());
...
function key() {
  ...
  return digest('roverbadge-super-v1\0' + process.env.SUPER_KEY);
}
export function sealSuper(kind, data, ttlSeconds) {
  ...
  return 'rbs1.' + Buffer.concat([iv, cipher.getAuthTag(), encrypted]).toString('base64url');
}
export function openSuper(kind, token) {
  try {
    if (typeof token !== 'string' || !token.startsWith('rbs1.') || token.length > 4096) return null;
