...
server.listen(PORT, '0.0.0.0', async () => {
  ...
  // 與 Vercel 行為對齊：開站即講明有效旅團，避免「綠燈但 /api 死咗」
  const { listPublicTroops } = await import('../api/_registry.js');
  const ids = Object.keys(listPublicTroops());
  console.log(`  有效旅團=${ids.length ? ids.join(',') : '⚠️ 無（proxy 會回 404）'}（API 面嚴格 4 個：proxy/troops/portal/super）`);
