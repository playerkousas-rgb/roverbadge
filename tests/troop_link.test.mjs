// 旅系統（進度系統節點：上下游接駁）守護測試
// 用 in-memory GAS stub 載入真實 apps-script/Code.gs，起兩個節點（上游／下游）經假網路對打，
// 驗證：HMAC sig 簽署與驗證、直接入口掣（ALLOW_LOCAL_LOGIN，未設定＝開啟、fail closed）、
//       開戶落下游（同一 password_hash）、匯出 JSON（含 hash）→ Drive 私人檔 → 匯入 upsertUser、
//       ABCD 四項登記資料只存 Script Properties、絕不寫入任何工作表。
...
const net = new Map();
// 假 Drive：同一個操作員的 Drive（搬舊數時，匯出檔要能被新節點讀到）
...
//   sigKey = HMAC-SHA256(message='roverbadge-troop-sig-v1', key=該節點 SHEET KEY) 的 hex
//   sig    = HMAC-SHA256(message=action\n ts\n nonce\n SHA256(rawBody), key=sigKey) 的 hex
const SIG_PURPOSE = 'roverbadge-troop-sig-v1';
...
// Users 表找行：回傳 { row, hash }（0-based row）
function findUsersRow(node, ymis) {
  const rows = sheetRows(node, 'Users');
  for (let i = 1; i < rows.length; i++) {
    if (String(rows[i][0] || '').trim() === ymis) return { row: rows[i], hash: String(rows[i][4] || '') };
  }
  return null;
}
...
const UPSTREAM_KEY = 'rv_upstream_branch_key_0001';
const DOWNSTREAM_KEY = 'rv_downstream_progress_key_0002';

function buildPair() {
  const up = makeNode({ name: '第 82 旅（支部系統）', apikey: UPSTREAM_KEY, url: UPSTREAM_URL });
  const down = makeNode({ name: '第 82 旅（進度）', apikey: DOWNSTREAM_KEY, url: DOWNSTREAM_URL });
...
test('直接入口掣：未設定時現有旅團行為完全不變', () => {
  const node = makeNode({ name: '獨立旅團', apikey: 'rv_standalone_key_0003', url: 'https://script.google.com/macros/s/STANDALONE_NODE/exec' });
  const g = node.context;
  g.initializeSheets();
  assert.equal(g.localLoginAllowed(), true, '未設定 ALLOW_LOCAL_LOGIN 應視為開啟');
  assert.equal(node.props.has('ALLOW_LOCAL_LOGIN'), false, 'initializeSheets 不應該自動寫入掣');
  ...
  // 舊 apikey 直寫路徑（gate 開放時）照舊
  const save = g.doPost({ parameter: {}, postData: { contents: JSON.stringify({ action: 'save', apikey: 'rv_standalone_key_0003', changes: [{ ymis: '1234567890', itemId: 'L1', date: '2026-01-01' }] }) } });
  assert.equal(save.success, true, '未閂口時 apikey save 行為不變');
});

test('閂口後：直接登入／申請／GET load／apikey save／token save 全部被拒，只收 sig', () => {
  ...
  const login2 = g.doPost({ parameter: {}, postData: { contents: JSON.stringify({ action: 'login', login_id: '1111111111', password: 'changeme' }) } });
  assert.equal(login2.success, false);
  assert.equal(login2.upstream_only, true);
  assert.match(login2.error, /ALLOW_LOCAL_LOGIN/);
  ...
  const saveKey = g.doPost({ parameter: {}, postData: { contents: JSON.stringify({ action: 'save', apikey: DOWNSTREAM_KEY, changes: [{ ymis: '1234567890', itemId: 'L1', date: '2026-01-01' }] }) } });
  assert.equal(saveKey.success, false, '閂口後只收 sig，apikey 直接寫入要被拒');
  const saveToken = g.doPost({ parameter: {}, postData: { contents: JSON.stringify({ action: 'save', token: login.token, changes: [{ ymis: '1234567890', itemId: 'L1', date: '2026-01-01' }] }) } });
  assert.equal(saveToken.success, false, '閂口後 token 直寫亦要被拒');
  // 串錯字／false／0／off 一律閂口（fail closed）
  down.props.set('ALLOW_LOCAL_LOGIN', 'False!');
  assert.equal(g.localLoginAllowed(), false, '串錯字要 fail closed');
  down.props.set('ALLOW_LOCAL_LOGIN', '0');
  assert.equal(g.localLoginAllowed(), false);
  down.props.set('ALLOW_LOCAL_LOGIN', 'yes');
  assert.equal(g.localLoginAllowed(), true, 'yes 視為開啟');
  g.setLocalLoginAllowed(false, 'test');
});
...
test('sig 防護：錯誤 key、竄改 body、過期時間戳、重放 nonce、白名單外 action 全部被拒', () => {
  ...
  down.context.setLocalLoginAllowed(false, 'test'); // 閂口後只收 sig：攻擊必須靠 sig 過關
  const body = { action: 'save', changes: [{ ymis: '1234567890', itemId: 'L1', date: '2026-03-03' }], on_behalf: '1111111111' };
  const raw = JSON.stringify(body);

  const wrongKey = signRequest(down, 'save', raw, 'rv_not_the_registered_key');
...
test('ABCD 四項登記資料只存 Script Properties，絕不寫入任何工作表', () => {
  ...
  for (const node of [up, down]) {
    ...
    // 進度系統唔設「支部」SHEET（成員名單的『支部』欄係既有 schema，保留）
    assert.equal(node.ss.getSheetByName('支部'), null, node.name + '：唔應該有支部工作表');
  }
  ...
  // 初始化後唔會自動出現支部工作表
  assert.equal(up.ss.getSheetByName('支部'), null);
  assert.equal(down.ss.getSheetByName('支部'), null);
});
...
test('開戶：上游揀團開戶，經 sig 落下游寫（兩邊同一 password_hash）', () => {
  ...
  const upstreamRow = findUsersRow(up, '2345678901');
  const downstreamRow = findUsersRow(down, '2345678901');
  assert.ok(upstreamRow && downstreamRow, '上下游都應有帳戶');
  assert.equal(upstreamRow.hash, downstreamRow.hash, '下游必須直插同一 hash');
  assert.equal(downstreamRow.hash, sha256('4321'));
  assert.equal(String(downstreamRow.row[3]), 'exec_committee');
  // 下游已閂口，但上游仍可直接寫下游進度
  ...
  assert.equal(String(findUsersRow(down, '2345678901').row[5]), '第 82 旅深資');
  ...
  assert.equal(String(findUsersRow(down, '2345678901').row[1]), '李四（改名）');
  assert.equal(String(findUsersRow(down, '2345678901').row[5]), '第 82 旅深資', '冇帶 branch 不可洗走既有小隊');
  assert.equal(findUsersRow(down, '2345678901').hash, sha256('4321'));
  assert.equal(sheetRows(down, 'Users').filter(r => String(r[0]) === '2345678901').length, 1, '不可出現重複列');
});

test('搬舊數：匯出含 hash（Drive 私人檔）→ 匯入逐個 upsertUser 直插 hash（保留舊密碼）', () => {
  const oldNode = makeNode({ name: '舊進度', apikey: 'rv_old_progress_key_0004', url: 'https://script.google.com/macros/s/OLD_PROGRESS_NODE/exec' });
  const newNode = makeNode({ name: '新進度', apikey: 'rv_new_progress_key_0005', url: 'https://script.google.com/macros/s/NEW_PROGRESS_NODE/exec' });
  oldNode.context.initializeSheets();
  newNode.context.initializeSheets();
  const manager = { ymis: '1111111111', name: '管理員', role: 'admin' };
  assert.equal(oldNode.context.addUser_({ ymis: '1234567890', name: '陳大文', email: 'chan@example.com', role: 'member', password: 'abcd' }, manager).success, true);
  assert.equal(oldNode.context.addUser_({ ymis: '2345678901', name: '李四', email: 'li@example.com', role: 'exec_committee', password: 'wxyz' }, manager).success, true);
  ...
  assert.equal(payload.format, 'roverbadge-users-export');
  ...
  // 新節點 initializeSheets() 已有內置管理員 → 同 YMIS 會被 upsert 更新而不是重複開戶
  assert.equal(imported.created, 2);
  assert.equal(imported.updated, 1);
  assert.equal(imported.failed, 0);
  const rec = findUsersRow(newNode, '1234567890');
  assert.equal(rec.hash, sha256('newpass1'), '舊密碼必須保留');
  assert.equal(rec.row[13] === true || String(rec.row[13]).toUpperCase() === 'TRUE', false, '搬舊數唔應該強制改密碼');
  // 舊密碼喺新節點直接可用
...
test('上游以 sig 批量匯入下游（importUsers）與讀取用戶清單（回應唔會洩漏 hash）', () => {
  const { up, down } = buildPair();
  up.context.registerDownstream('progress', DOWNSTREAM_URL, DOWNSTREAM_KEY, '進度節點');
  const users = [
...
test('上游傳來的標籤不可變成工作表算式（auth_by／操作紀錄）', () => {
  ...
  const rec = findUsersRow(down, '1234567890');
  assert.match(String(rec.row[7]), /^[0-9A-Za-z_.@-]+$/, 'auth_by 只留安全字元');
});
