...
- 成員的 YMIS 必須為 10 位數字，否則該列會被忽略（YMIS PDF 匯入可用「🔢 全部補零至10位」輔助）。
- 領袖列（`role` 非 member）可留空 YMIS，但必須有姓名＋電郵；系統會自動編配內部編號，用電郵登入。
- 已存在的 YMIS 會被跳過（不覆蓋）。
- 團長全團只可一位；CSV 內若已有在職團長，再匯入 `group_leader` 會失敗並彈出原因。
- 密碼留空或 1234 時，該用戶首次登入必須更改密碼。
